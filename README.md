...
3. 手動で再生成・チェックする場合:
   ...
   0.2 mm ごとに輪切りにして、宙に浮いた部分・長すぎるブリッジ・片持ちの張り出しも場所付きで報告する
   （例: `z=5.1 (x=60.1, y=0.0) 長さ 4.0 mm`。Bambu Studio のプレビューでその高さを確認するとよい）。
...
2. Claude が E2E（書き出し → チェック → レンダリング → ビューア生成）まで実行し、会話に次のファイルを送ってくる。
   - `<name>-viewer.html` … サイドパネルで開くと、**ドラッグで回転・ピンチで拡大**できる 3D ビューア。寸法とチェック結果も表示される。
     複数パーツならパーツごとの表示／半透明、可動部（`motions`）があれば**角度スライダー・「揺らす」再生・干渉表示**、断面表示も使える
...
1. `check_stl.py` が **ERROR 0** であることを確認する（WARN は spec.md に理由があるか確認）。
   spec.md に「試し刷り」があれば、先に `out/<name>-coupon-*.stl` を刷ってはめあいを決める。
...
6. スライスしてプレビューで確認（ブリッジ・オーバーハング・最初の層）し、LAN 経由でプリンタへ送信して印刷する。
   check_stl が WARN を出した高さ（例: `z=5.1`）はプレビューで重点的に見る。
   - 底面外周はモデル側で 0.5 mm 面取りしてあるので、Bambu Studio のエレファントフット補正は 0 か小さめにする
   - 反りが出たら（大きい平板・PETG で角が浮く等）ブリム 5 mm 程度を足す
